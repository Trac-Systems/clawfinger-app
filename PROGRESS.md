...
## 2026-02-21 22:59 — Capture-path hardening for clipped/prefixed turns

### Problem observed
- Mixed regression pattern in live calls: some turns clipped at start, others carried long non-speech prefix before user speech.
- Runtime logs showed occasional drop from state-machine stream path into legacy fallback probes, plus route churn around no-audio windows.

### Fixes applied
- In strict-stream mode, hard-blocked fallback probe loop when no utterance is assembled:
  - no legacy `tinycap` probe retry in strict path,
  - immediate retry via capture loop only.
- Added fast-window pre-roll reduction in state-machine capture:
  - normal pre-roll remains unchanged,
  - post-playback fast window now uses shorter pre-roll budget to reduce prefix noise.
- Added leading pre-speech trim on assembled PCM before ASR:
  - scans early audio with VAD/RMS,
  - trims only leading non-speech while keeping guard audio before detected speech.
- Updated build marker to confirm runtime binary version:
  - `20260221-stream-capture-trim-a`.

### Deploy
- Rebuilt debug APK and installed to device.
- Re-pushed runtime profile and force-stopped app so next call reloads profile.

## 2026-02-21 23:12 — Soft-start capture guard for clipped prefixes

### Symptom in latest call before fix
- Some turns still dropped opening words (example: user said longer prompt, ASR began mid-phrase).
- Endpoint/rate remained stable (`capture d20 @ 24k`), so issue was capture-start sensitivity, not pitch drift.

### Change applied
- Added low-energy soft-start detection to utterance state machine:
  - if VAD does not trigger yet but RMS stays above soft threshold for a short streak, start utterance capture anyway,
  - preserves quiet sentence starts without waiting for stronger voiced detection.
- Build marker bumped to: `20260221-capture-softstart-a`.
- Commit: `0d451a7`.

### Deploy
- APK rebuilt + installed.
- Profile re-pushed + app restarted.
- Validation dial triggered to `+4915129135779`.

## 2026-02-21 23:34 — Analysis + stability fix after latest clipped run

### Observed in latest run (`debug-wavs/latest-call-20260221-233124`)
- Turn 1 ASR was correct.
- Turn 2 dropped leading words (`"A good book."`).
- Shortly after, capture unpinned from `d20` and rotated to `d21` on `fast_post_playback_no_audio_8`.
- `d21` produced empty/garbled turns, causing no-response behavior.

### Root cause (current)
- No-audio recovery/unpin/rotation triggers too early during normal post-reply listening windows.
- That recovery path can switch away from working capture endpoint before/while user starts speaking.

### Fix applied
- Increased no-audio thresholds to avoid premature recovery churn:
  - `STARTUP_NO_AUDIO_UNPIN_THRESHOLD`: `4 -> 10`
  - `STARTUP_CAPTURE_SOURCE_ROTATE_THRESHOLD`: `4 -> 12`
  - `STARTUP_FAST_POST_PLAYBACK_STREAM_UNPIN_THRESHOLD`: `4 -> 12`
  - `NO_AUDIO_UNPIN_THRESHOLD`: `3 -> 8`
  - `ROOT_CAPTURE_SOURCE_ROTATE_THRESHOLD`: `3 -> 10`
  - `FAST_POST_PLAYBACK_STREAM_UNPIN_THRESHOLD`: `4 -> 12`
  - `UTTERANCE_NO_SPEECH_TIMEOUT_MS`: `2200 -> 4200`
- Build marker updated to `20260221-capture-stability-b`.
- Commit: `fecd278`.

## 2026-02-21 23:36 — Current blocking problem (still open)

### Current user-facing failure
- Greeting and first turn usually work.
- After greeting/turn 1, subsequent turns still frequently clip the opening words.
- In bad runs, turn 2+ can degrade to no-response behavior.

### Latest symptom examples
- Spoken: `can you recommend me a good book` → captured/transcribed as `A good book.`
- Spoken: `can you recommend me a good movie` → later turns can still lose prefix or stall.

### Why this is still unresolved
- Capture path remains sensitive in the first seconds after assistant playback ends.
- Recovery + endpoint handling can still drift into weaker capture behavior under no-audio streaks.

### Immediate next debugging focus
- Keep endpoint stability while isolating post-playback listen window behavior.
- Validate whether the capture stream starts late vs. starts on time but misses low-energy initial phonemes.
- Use latest-call WAV+log correlation per turn to verify exact cut point before applying further code changes.

## 2026-02-21 23:58 — Post-playback capture stability experiments (partial improvement, blocker still open)

### What
- Updated `GatewayCallAssistantService` state-machine recovery behavior:
  - fast post-playback mode now expires dynamically based on real time (instead of being fixed for the whole loop),
  - pre-speech no-audio no longer triggers endpoint unpin/rotation,
  - added timeout-path same-endpoint stream rebind and timeout-path route reapply (without endpoint drift),
  - added explicit FIFO prep failure logs for root capture/playback stream startup.
- Applied temporary stability profile pin for capture candidates:
  - `profiles/pixel10pro-blazer-profile-v1.json` capture order set to `[20]` (keep `d21` out of active rotation while blocker is open).
- Rebuilt, installed, and redeployed profile for each iteration.

### Why
- User-reported evidence showed that post-turn behavior was still unstable:
  - `rxm-d21-r24000-1771714115243-vad-90.wav` captured mostly non-user/background content and was perceived as recording while assistant was still talking,
  - `rxm-d21-r24000-1771714127735-vad-37.wav` had clipped opening.
- We needed to stop no-audio recovery from drifting into weaker endpoints while preserving a safe way to recover stalled streams.

### Result
- Confirmed deployed build marker is still `20260221-capture-stability-b` during these runs.
- Run `debug-wavs/latest-call-20260221-234747` (before `d20`-only pin effect):
  - turn 1 captured correctly on `d20`,
  - then unpinned to `d21` and produced unstable/noisy capture behavior.
- Runs `debug-wavs/latest-call-20260221-235224`, `...-235446`, `...-235708` (with `d20`-only pin):
  - no drift to `d21`,
  - first turn captured/transcribed correctly,
  - subsequent turns still showed repeated `state-machine utterance empty; strict stream retry` and no follow-up assistant reply before disconnect in these attempts.
- Net: endpoint drift reduced, but turn-2+ reliability is still not solved.

### Next
1. Keep `d20` pinned until multi-turn turn-2+ capture reliability is restored.
2. Continue tuning no-speech timeout/rebind cadence to eliminate 4-5s dead windows after first reply.
3. Validate with explicit scripted 3-turn prompts and per-turn WAV/log correlation.
4. After blocker is resolved, resume planned PCM training/testing on channels `22`, `54`, and additional endpoints.

## 2026-02-22 00:19 — Post-playback backlog flush fix for turn-2 clipping/high-pitch capture

### Observed pattern (from latest pulled runs + logs)
- Capture stream was kept alive across assistant playback, then next listen window could start by reading stale FIFO audio from playback period.
- This matches user-reported artifacts:
  - turn-2 start clipping,
  - occasional "recorded while AI was talking" files,
  - intermittent high-pitch/non-user segments in `rxm-*`.

### Code changes applied (`GatewayCallAssistantService`)
- Added explicit post-playback capture backlog handling:
  - new state flag: `postPlaybackCaptureFlushPending`.
  - mark pending after greeting/reply playback completion and interruption paths.
  - mark pending after ready-beep playback.
- At utterance-state-machine start (first stream-chunk cycle per turn):
  - if pending or still within a short post-playback window, clear rolling prebuffer and flush queued bytes from the root capture FIFO before consuming turn audio.
  - added structured audit/log events for backlog flush amount + reason.
- Lifecycle hygiene:
  - reset pending flush state on service start/destroy.
- Kept strict empty-utterance retry behavior from current branch.
- Restored `UTTERANCE_NO_SPEECH_TIMEOUT_MS` to `4200` while validating this capture-backlog fix.

### Build/deploy
- Debug APK rebuilt successfully.
- APK installed to device.
- Profile re-pushed (`profiles/pixel10pro-blazer-profile-v1.json`) and app force-stopped to reload on next call.

### Validation status
- Build-time validation: PASS.
- Live acceptance helper script failed in this workspace because `phone/node-control/src/cli.js` is missing.
- Next validation step is a manual call run to `+4915129135779` and artifact pull (`latest-call-*`) to confirm:
  - no assistant-playback bleed into next `rxm-*`,
  - reduced turn-2 clipping,
  - stable multi-turn understanding.

## 2026-02-22 00:22 — Restore capture candidate order to 20,21

### Correction
- Reverted profile capture candidate order from `[20]` back to `[20, 21]` in:
  - `profiles/pixel10pro-blazer-profile-v1.json`

### Why
- Keeping only `20` is too restrictive and can degrade recovery when endpoint behavior shifts mid-call.
- Stable baseline for this project phase is `20,21` before broader endpoint work resumes.

### Deploy
- Profile will be pushed to device immediately after commit.

## 2026-02-22 00:34 — Stability cleanup decisions applied after full code review

### Decision outcomes (owner: AI)
1. `no_unvalidated_endpoint_fallback` should NOT force strict-stream mode globally.
2. Route recovery should NOT run during active utterance capture turns (only outside-turn or forced failures).
3. Short-turn consensus gate should be disabled for now; it is too aggressive during stability recovery.

### Code changes (`GatewayCallAssistantService.kt`)
- Strict-mode semantics cleanup:
  - turn-loop `strictStreamOnly` now follows `runtimeStrictStreamOnly` only.
  - policy flag no longer hard-forces strict stream behavior in state-machine empty handling.
- Recovery concurrency guard:
  - added `captureTurnInProgress` flag around `captureUtteranceStateMachine()`.
  - `maybeRecoverRootRoute(force=false, ...)` now skips when capture turn is active.
- Short-turn rejection disabled:
  - added `ENABLE_SHORT_TURN_CONSENSUS=false` and gated consensus rejection path behind it.
- Startup profile hard-fail fix:
  - added `runtimeProfileLoaded` state.
  - `loadRuntimePcmProfile()` sets this explicitly.
  - service start now aborts early (without continuing route/capture setup) if runtime profile is invalid/unavailable.
- Build marker updated to `20260222-stability-cleanup-a`.

### Deploy/validation
- Debug APK rebuilt successfully.
- APK installed.
- Profile pushed and app force-stopped so next call reloads runtime profile.

### Notes
- This is a logic-stability cleanup pass; it intentionally avoids endpoint retuning changes.
- Remaining larger technical debt from review (manifest/lint/security/dead-path cleanup) is still pending.

## 2026-02-22 00:40 — Cleanup-only pass after review (no call experiments)

### What was cleaned
- `GatewayCallAssistantService`:
  - Removed dead/unused controls and constants (unused post-playback unpin helper + stale unused constants).
  - Added explicit `RECORD_AUDIO` runtime-permission guard before AudioRecord fallback capture path.
  - Kept strict-stream semantics cleanup from previous pass (`runtimeStrictStreamOnly` governs strictness).
- `AndroidTelecomController`:
  - Fixed wrong `answer(...)` argument to `VideoProfile.STATE_AUDIO_ONLY`.
  - Added explicit permission checks for dialing path (`CALL_PHONE` and guarded `READ_PHONE_STATE`).
- `AndroidManifest.xml`:
  - Removed privileged/system-only `CAPTURE_AUDIO_OUTPUT` permission.
  - Added telephony feature declaration with `required=false`.
  - Reduced foreground service type for assistant service to `microphone` to align with declared permissions.

### Validation
- `./scripts/android-build-debug.sh` -> PASS.
- `:app:lintDebug` improved from prior hard failures (7 errors) to clean error-free run after this pass.

### Notes
- This pass intentionally avoided new call runs and endpoint tuning.
- Focus was code health + correctness cleanup before further behavioral experiments.

## 2026-02-22 01:14 — Capture/playback flow cleanup after full review (no call run)

### Scope
- Completed a structural cleanup pass focused on post-playback capture correctness and scheduler determinism in `GatewayCallAssistantService`.
- No endpoint retuning and no live call experiments in this pass.

### Changes applied
- Removed post-playback capture prearm behavior from playback monitor:
  - deleted prearm timing loop and prearm chunk read path during assistant playback.
  - removed related constants (`ENABLE_POST_PLAYBACK_CAPTURE_PREARM`, `POST_PLAYBACK_PREARM_*`).
- Added hard capture reset before every root playback (`playReplyViaRootPcm`):
  - stop active root capture stream,
  - clear rolling prebuffer,
  - clear pending post-playback flush marker,
  - audit event `voice_bridge:capture_reset_pre_playback:*`.
- Reworked capture-loop scheduling to generation-based dedupe:
  - added `captureLoopGeneration` token,
  - stale scheduled loops/beep callbacks now self-cancel,
  - prevents overlapping old capture-loop callbacks from racing after turn transitions.
- Removed dead/unused state from service:
  - unused runtime knobs (`runtimeCaptureDurationByAttemptMs`, `runtimeUtteranceMinSpeechMs`, `runtimeFastPostPlaybackSilenceMs`, `runtimeLocalTranscriptHintEnabled`),
  - unused no-arg `maybeRecoverRootRoute()`,
  - unused `RootCaptureStreamSession.pendingData` buffering path,
  - redundant `appendChunk` branch in state-machine write path.
- Updated build marker to `20260222-capture-flow-cleanup-a`.

### Why
- User symptoms indicated capture contamination around playback boundaries (assistant playback bleed + clipped turn starts), and unstable turn-2+ behavior consistent with stale scheduler callbacks.
- Prearming capture while assistant playback is active is contradictory to clean turn separation.

### Validation
- `./scripts/android-build-debug.sh` => PASS.
- Build remains clean except prior non-blocking deprecation warning on `stat_sys_phone_call` icon.

### Next
1. Install and profile-push this build, then run one controlled 3-turn manual call to `+4915129135779`.
2. Pull latest call artifacts and verify:
   - no assistant-playback bleed in `rxm-*`,
   - no clipped prefixes on turn 2/3,
   - no no-response dead window after first reply.
3. If stable, continue planned capture endpoint training on channels `22`, `54`, then additional candidates.

## 2026-02-22 00:53 — Live run analysis: still blocked after turn 1 + targeted hotfix

### User-observed failure (confirmed)
- User reported: could not get past turn 1; call hung up after no response.

### Pulled artifacts (this run)
- Directory: `debug-wavs/latest-call-20260222-004933`
- WAVs:
  - `rxm-d20-r24000-1771717789198-vad-28.wav`
  - `tx-d29-1771717790061-can-you-recommend-me-a-good-book.wav`
- Transcripts (`transcripts.txt`):
  - `rxm...`: `Can you recommend me a good book?`
  - `tx...`: assistant reply decoded as expected.

### Log correlation (key)
- Build marker active: `20260222-capture-flow-cleanup-a` (before hotfix below).
- Turn 1 path was good:
  - capture on `d20` succeeded,
  - ASR transcript correct,
  - assistant playback succeeded.
- Turn 2 failure path:
  - `state-machine utterance empty; falling back to capture probes (strictStream=false)`
  - fallback attempt 1 on `d20` -> `root tinycap legacy empty ... Invalid argument`
  - `audio fallback capture rejected: no_audio_source`
  - `root capture source unpinned: no_audio_source_streak_13`
  - `capture shift d20 -> d21` (degradation path)
- This confirms blocker is no longer turn-1 clipping in this run; blocker is turn-2 empty->fallback probe path causing unpin/rotate and no-response behavior.

### Hotfix applied
- In `GatewayCallAssistantService.requestReplyFromAudioFallback()`:
  - if `useStateMachine && lastRejectionReason == "utterance_empty"`, now force stream retry path and skip fallback probe loop.
  - audit marker added: `voice_bridge:state_machine_empty_retry`.
- Build marker bumped to: `20260222-turn2-streamretry-a`.

### Validation
- `./scripts/android-build-debug.sh` -> PASS.

### Next
1. Install + push profile.
2. Re-run live call to verify turn-2 no longer enters fallback no-audio rotate path.
3. If still blocked, next patch will reset `consecutiveNoAudioRejects` on state-machine-empty boundary and block unpin/rotate escalation from fallback when prior failure was pre-speech empty-stream.

## 2026-02-22 00:55 — Validation call dispatch after turn-2 stream-retry hotfix

### What
- Installed latest debug APK (`build marker: 20260222-turn2-streamretry-a`), pushed profile, force-stopped app.
- Triggered validation dial to `+4915129135779`.

### Initial status
- Logs confirmed command + call progression: `Dial command executed`, `state=connecting`, `state=dialing`.

### Next
- Await user runtime result, then pull WAV + focused logs and correlate turn-2 behavior under this patch.

## 2026-02-22 00:58 — Turn-2 empty-capture follow-up after user retest failure

### Observed from latest failed run (user confirmed)
- Build marker during run: `20260222-turn2-streamretry-a`.
- Turn 1 still succeeds on `d20` with correct transcript.
- Turn 2 still fails, but failure shape changed:
  - repeated `state-machine utterance empty` events,
  - no immediate fallback probe rejection/unpin logs on first empty cycle,
  - call eventually ends before a successful second user capture.
- New files from this run:
  - `rxm-d20-r24000-1771718127700-vad-35.wav`
  - `tx-d29-1771718128588-can-you-recommend-a-good-movie.wav`

### Interpretation
- Prior patch did prevent the worst fallback drift path for state-machine empty turns.
- Remaining blocker is likely post-playback startup sensitivity where first user audio after assistant reply can be lost/consumed before speech lock.

### Follow-up hotfix applied
- Added post-playback flush skip when capture stream was explicitly reset before playback:
  - new flag: `skipNextPostPlaybackBacklogFlush`.
  - set in `resetCaptureStreamBeforePlayback(...)`.
  - in state-machine startup, if skip flag is set, pending backlog flush is skipped once and only flags are cleared.
- Reset `consecutiveNoAudioRejects` when state-machine returns `utterance_empty`.
- Updated non-strict log text from misleading fallback wording to explicit stream-retry wording.
- Build marker bumped to: `20260222-turn2-backlogskip-a`.

### Validation
- `./scripts/android-build-debug.sh` -> PASS.

### Next
1. Deploy this build + profile.
2. Re-run live call on `+4915129135779`.
3. Check whether turn-2 still produces `utterance_empty` without `rxm-*` emission.

## 2026-02-22 — Keep capture stream alive during playback (turn-2 fix)

### Root cause analysis
- `resetCaptureStreamBeforePlayback()` was calling `stopRootCaptureStreamSession()` before every reply playback.
- After reply finished, turn-2 had to re-open the capture FIFO. Re-opening could fail or return empty chunks.
- This caused `captureUtteranceStateMachine()` to timeout (4200ms no-speech) → return null → retry after 450ms → same failure → dead loop until caller hangs up.
- Turn 1 worked because the stream opened fresh after greeting and had enough timing margin.

### Fix applied
- Removed `stopRootCaptureStreamSession()` from `resetCaptureStreamBeforePlayback()`.
- Stream stays alive during playback; FIFO fills with playback-period audio.
...
## 2026-02-22 03:20 — spark2 ngrok basic auth removed for voice gateway
...
## 2026-02-22 15:35 — Spark gateway `/api/turn` supports `forced_reply` bypass
