...
- Root method: **APatch** (expected to work — validated on Pixel 10 Pro, same flow).
...
- Build: determine at preflight — run `adb shell getprop ro.build.id` and download the matching factory image from https://developers.google.com/android/images#lynx
...
- note the exact build ID — factory image must match
- `unlocked: yes`
...
# Confirm on device, then reboot and re-enable USB debugging
```
...
# Download from: https://developers.google.com/android/images#lynx
unzip -o lynx-*.zip
